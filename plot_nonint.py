...
for block, archivename in itt.product([('up','up'), ('up','dn')], sys.argv[1:]):
    for w in range(2):
        fig, [[ax1, ax2], [ax3, ax4]] = plt.subplots(2,2)
        plot_kwargs = {'vmin': -2, 'vmax': 2, 'aspect':'equal'}
        for blockstruct, ax in zip(["AABB", "ABBA"], [ax1, ax3]):
            sto = HDFArchive(archivename, 'r')
            g0 = sto["G0_iw"]
            g2_0 = G2_0(g0, 10, 15, [], blockstruct)
        ...
        outname = archivename[:-3]+"_g2inuinup_"+block[0]+block[1]+"_w"+str(w)+".pdf"
        plt.savefig(outname)
        print outname+" ready"
        plt.close()
    ...
    for blockstruct, ax in zip(["AABB", "ABBA"], [ax1, ax2]):
        ...
        ax.set_xlim(left= 0)
    outname = archivename[:-3]+"_g2iw_"+block[0]+block[1]+".pdf"
    plt.savefig(outname)
    print outname+" ready"
    plt.close()
